...
export class World extends Phaser.Scene {
  ...
  create() {
    ...
    for (let i = 0; i < this.easyMaze; i++) {
      const mazeConfig = GenerateMazeConfig("Maze_easy_"+i, this.worldTitle, Difficulty.EASY, this.enableDeadWalls, this.enableEnemies);
      const x = this.scale.width / 4 - 50;
      const y = 100 + i * 150;
      createMazeButton(this, x, y, mazeConfig);
      // leaderboard
      this.add.text(x - 95, y + 30, `Leaderboard:`, { fontSize: '16px', color: '#fff' });
      scores
        .filter(score => score.mazeName === mazeConfig.title)
        .sort((a, b) => b.score - a.score)
        .forEach((score, index) => {
          this.add.text(x - 95, y + 50 + index * 20, `${score.name}: ${score.score}`, { fontSize: '16px', color: '#fff' });
        });
    }
    for (let i = 0; i < this.mediumMaze; i++) {
      const mazeConfig = GenerateMazeConfig("Maze_medium_"+i, this.worldTitle, Difficulty.MEDIUM, this.enableDeadWalls, this.enableEnemies);
    ...
    for (let i = 0; i < this.hardMaze; i++) {
      const mazeConfig = GenerateMazeConfig("Maze_hard_"+i, this.worldTitle, Difficulty.HARD, this.enableDeadWalls, this.enableEnemies);
    ...
    createButton(this, this.scale.width*0.8, this.scale.height*0.9, 'Main Menu', 'MainMenuScene', false);
  }
}
